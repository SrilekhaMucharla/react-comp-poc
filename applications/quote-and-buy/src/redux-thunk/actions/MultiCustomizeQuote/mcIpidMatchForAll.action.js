...
export const getMCIpidMatchForAll = (ipadObject) => (dispatch) => {
    dispatch({
        type: IPID_MC_MATCHFORALL_START
    });
    HastingsIpidService.mcIpidByProducerCode(ipadObject)
        .then(({ result }) => {
            dispatch({
                type: IPID_MC_MATCHFORALL_SUCCESS,
                payload: result
            });
            dispatch({
                type: MC_UPDATE_IPIDS_INFO_ANCILLARY_DOCUMENT_LINKS,
                payload: result.ipidResponse
            });
            // trackAPICallSuccess('Ipid By Producer Code');
        }).catch((error) => {
            dispatch(setErrorStatusCode(error.status));
            dispatch({
                type: IPID_MC_MATCHFORALL_FAIL,
                payload: error
            });
...
export const getMCIpidDocument = (ipadObject, pageName) => (dispatch) => {
    dispatch({
