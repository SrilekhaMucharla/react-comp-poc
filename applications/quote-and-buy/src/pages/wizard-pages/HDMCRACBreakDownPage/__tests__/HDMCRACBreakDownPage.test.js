...
import HDMCRACBreakDownPage from '../HDMCRACBreakDownPage';
...
describe('<HDMCRACBreakDownPage />', () => {
    ...
    const indexArr = [{ indx: 0, quoteID: '1234' }];
    afterEach(() => {
        if (wrapper) {
            wrapper.unmount();
            wrapper = undefined;
    ...
    test('Component should set state and COntainer Row', async () => {
        ...
        wrapper = mount(
            <Provider store={store}>
                <HDMCRACBreakDownPage indexArray={indexArr} pageMetadata={pageMetadata} mcsubmissionVM={multisubmissionVM} setTotalPrice={setTotalPrice} />
            </Provider>
        );
        expect(wrapper.find('.mc-rac-container-row').exists()).toBeFalsy();
    });
    ...
    it('should contain HDModal', async () => {
        ...
        expect(wrapper.find(HDModal).exists()).toBe(true);
    });
});
